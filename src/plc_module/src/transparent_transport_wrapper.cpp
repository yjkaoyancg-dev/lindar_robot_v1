#include "plc_module/transparent_transport_wrapper.h"
...
namespace {
void log_tt_warn(const std::string& msg) {
  std::cerr << "[transparent_transport][WARN] " << msg << std::endl;
}

void log_tt_error(const std::string& msg) {
  std::cerr << "[transparent_transport][ERROR] " << msg << std::endl;
}
}  // namespace
...
TransparentTransportWrapper::TransparentTransportWrapper(const std::string& ip,
                                                         int port)
    : ip_(ip), port_(port), slave_id_(1), socket_(io_) {}

TransparentTransportWrapper::~TransparentTransportWrapper() { disconnect(); }

void TransparentTransportWrapper::connect(int slave_id) {
  slave_id_ = slave_id;
...
void TransparentTransportWrapper::disconnect() {
  if (socket_.is_open()) socket_.close();
}

void TransparentTransportWrapper::setSlaveId(int id) const { slave_id_ = id; }

std::string TransparentTransportWrapper::info() const {
...
bool TransparentTransportWrapper::readHoldingRegisters(
    int address, int count, std::vector<uint16_t>& out) {
  return readRegisters(0x03, address, count, out);
}

bool TransparentTransportWrapper::readInputRegisters(
    int address, int count, std::vector<uint16_t>& out) {
  return readRegisters(0x04, address, count, out);
}

bool TransparentTransportWrapper::writeRegisters(
    int address, const std::vector<uint16_t>& data) {
  ...
  try {
    ...
    if (function == 0x06) {
      ...
      if (data.size() > 123) {
        log_tt_error("写入寄存器数量超过 123");
        return false;
      }
      size_t byte_count = data.size() * 2;
      if (byte_count > 255) {
        log_tt_error("byte_count 超过 255");
        return false;
      }
    ...
    read(socket_, buffer(response));

    // CRC 校验（对前 6 字节算 CRC）
    uint16_t crc_recv = response[6] | (response[7] << 8);
    uint16_t crc_calc = compute_modbus_crc16(response.data(), 6);
    if (crc_recv != crc_calc) {
      log_tt_error("写寄存器 CRC 错误");
      return false;
    }

    // 功能码校验
    if (response[0] != (uint8_t)slave_id_) {
      log_tt_error("从站ID不匹配");
      return false;
    }
    if (response[1] != function) {
      std::ostringstream oss;
      oss << "功能码不匹配，期望 0x" << std::hex << int(function)
          << " 实际 0x" << int(response[1]) << std::dec;
      log_tt_error(oss.str());
      return false;
    }
    ...
    log_tt_error(std::string("写寄存器失败: ") + e.what());
    return false;
  }
}


bool TransparentTransportWrapper::writeCoil(int address, bool value) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  try {
    ...
    log_tt_error(std::string("写单个线圈失败: ") + e.what());
    return false;
  }
}

bool TransparentTransportWrapper::readRegisters(uint8_t function_code,
  ...
  try {
    ...
    read(socket_, buffer(header));

    if (header[0] != slave_id_ || header[1] != function_code) {
      log_tt_error("地址或功能码不匹配");
      return false;
    }

    int byte_count = header[2];
    if (byte_count != count * 2) {
      log_tt_error("响应长度异常: byte_count=" + std::to_string(byte_count));
      return false;
    }

    // 接收数据 + CRC
    std::vector<uint8_t> data_crc(byte_count + 2);
    read(socket_, buffer(data_crc));
    ...
    if (crc_recv != crc_calc) {
      std::ostringstream oss;
      oss << "CRC校验失败: recv=" << std::hex << crc_recv << ", calc="
          << crc_calc << std::dec;
      log_tt_error(oss.str());
      return false;
    }
    ...
    log_tt_error(std::string("Modbus读取失败: ") + e.what());
    return false;
  }
}
